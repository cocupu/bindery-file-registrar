...
FileSystemWalker.walkAndRegister = function (dir, db, emitter, done) {
  ...
  fs.readdir(dir, function(err, childPaths) {
    if (err) {
    ...
    fs.stat(dir, function(err, stat) {
      entryInfo = writeDirectoryInfo(dir, stat, childPaths, db, emitter)
    })
    var i = 0;
    (function next() {
      var childBaseName = childPaths[i++];
      if (!childBaseName) {
...
function writeDirectoryInfo (dir, stat, childPaths, db, emitter) {
  var entryKey = Identifiers.entryIdFor(dir, stat.dev)
  entryInfo = {type: 'dir', size: stat.size, mtime: stat.mtime, birthtime: stat.birthtime, path: dir, storagePlatform: 'localFs', deviceId: stat.dev, children:childPaths}
  db.put(entryKey, JSON.stringify(entryInfo), function (err) {
    if (err) return console.log('Ooops!', err)
